...
import shap
from source.hyper import DataLoader, ModelTrainer, GeneticAlgorithm, Visualization, NeuralNetworkTrainer
...
class CreditRiskAnalysisApp:
    ...
    def run(self):
        ...
        if uploaded_file is not None:
            ...
            data = self.data_loader.load_data(uploaded_file, value=["csv"])
            ...
            with tab1:
                ...
                if button_optimize:
                    ...
                    # Preprocess data
                    X, y = self.data_loader.preprocess_data(data, target_column)
                    ...
                    if result is not None:
                        best_model_params, best_auc_roc, initial_hyperparameters, optimized_hyperparameters = result

                        # Plot line plot
                        self.visualization.plot_line_plot(initial_hyperparameters, optimized_hyperparameters, hyperparameters)

                        # Plot bar chart
                        self.visualization.plot_bar_chart(initial_hyperparameters, optimized_hyperparameters, hyperparameters)

                        # Plot heatmap
                        self.visualization.plot_heatmap(initial_hyperparameters, optimized_hyperparameters, hyperparameters)
            ...
            with tab2:
                ...
                if button_train:
                    ...
                    if hyperparams_optimized:
                        ...
                        pd_model = self.model_trainer.train_pd_model(X_train, y_train, best_model_params)
                        accuracy, precision, recall, f1, auc_roc = self.model_trainer.evaluate_pd_model(pd_model, X_test, y_test)
            ...
            with tab3:
                ...
                if button_benchmark:
                    ...
                    initial_model = self.model_trainer.train_pd_model(X_train, y_train)

                    # Evaluate initial model
                    initial_metrics = self.model_trainer.evaluate_pd_model(initial_model, X_val, y_val)

                    # Evaluate optimized model
                    optimized_model = self.model_trainer.train_pd_model(X_train, y_train, best_model_params)
                    optimized_metrics = self.model_trainer.evaluate_pd_model(optimized_model, X_val, y_val)
